...
router.get("/settings", catchAsync(async (_: express.Request, res: express.Response) => {
  try {
...
router.put("/settings", catchAsync(async (req: express.Request, res: express.Response) => {
...
router.delete("/settings/assignment/:id", catchAsync(async (req: express.Request, res: express.Response) => {
    const token = req.headers["authorization"]?.replace("Bearer ", "");
    if (!token || !Crypto.validToken(token)) {
      res.status(401).type("json").send({ message: "Token is invalid" });
      return;
