...
export default class Crypto {
    static cipher(phrase: any, thumbprint: string = Constants.CIPHER_KEY): string {
        const key = crypto.createHash("sha256").update(`#$£@${thumbprint}+=;?`).digest();
    ...
    static decipher(phrase: string): any {
        const key = crypto.createHash("sha256").update(`#$£@${Constants.CIPHER_KEY}+=;?`).digest();
    ...
    static validToken(token: string) {
        try {
            ...
            const diff = new Date().getTime() - decipher.date;
            if (diff < 24 * 60 * 60 * 1000 && decipher.admin === true) { // token valid for 24 hours
                return true;
            }
        } catch (_) {}
        return false;
